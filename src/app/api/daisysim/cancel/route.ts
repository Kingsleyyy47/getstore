...
export async function POST(req: Request) {
  ...
  try {
    ...
    // ~2-minute TOO_EARLY window, so this branch should be unreachable in
    // normal operation -- kept only as a defensive fallback.
    if (e instanceof daisysim.DaisySimError && e.code === "TOO_EARLY") {
      return NextResponse.json(
        { error: "You can't cancel this number just yet -- please wait a bit and try again." },
        { status: 422 }
      );
    }
  ...
  // DaisySim has now CONFIRMED the cancellation. Flip status atomically
